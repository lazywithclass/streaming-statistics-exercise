...
printer.print = function(input) {
  ...
  var fieldOutput = util.format(
    fieldTemplate,
    shortest.sessionId, longest.sessionId, textAverage.sessionId.average.toFixed(3),
    shortest.page, longest.page, textAverage.page.average.toFixed(3),
    d(min.latency), max.latency, numberAverage.latency.average.toFixed(3),
    d(min.timeOnPage), max.timeOnPage, numberAverage.timeOnPage.average.toFixed(3)
  );
...
var textRunningAverage = {
  sessionId: { count: 0, sum: 0, average: 0 },
  page: { count: 0, sum: 0, average: 0 }
};
